...
function getNumberOfTrends(cycle) {
  ...
  if (r != undefined) {
    ...
    if (j != undefined) {
      return j
        .map((x) => Object.values(x[1]).filter((y) => y != 0).length)
...
function renderPopup(cyc) {
  ...
  document.getElementById("avgintensity").innerText =
    cyc.events.length > 0
      ? Math.round(
          (Analyze.extractIntensities(cyc.events).reduce((a, b) => a + b) /
            cyc.events.length) *
            100,
        ) / 100
      : "No logs";
  ...
  if (cyc.events.length > 0) {
    ...
    document.getElementById("dmax").innerText = ``;
    document.getElementById("barchart").innerHTML =
      `<button class="bar" style="width:100%; height:100%; background-color:rgba(0, 0, 0, 0.2)"></button>`;
  }

  document.getElementById("pottrends").innerText = document
    .getElementById("allTrends")
    .className.includes("formalive")
    ? document.getElementById("pottrends").innerText
    : `${cyc.events.length > 0 ? getNumberOfTrends(cyc) : "0"} potential trend${getNumberOfTrends(cyc) == 1 ? "" : "s"} found`;
  ...
  if (cyc.events.length > 1) {
    ...
    if (preds != undefined) {
      if (preds[0] != undefined) {
        let d = new Date(
          (cyc.events[cyc.events.length - 1].date + preds[0]) * 60 * 1000,
        );
        document.getElementById("datepredict").innerText =
          `Date: ${["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"][d.getDay()]}, ${["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][d.getMonth()]} ${d.getDate()}, ${d.getFullYear()} at ${d.getHours() == 0 ? 12 : d.getHours() % 12} ${d.getHours() < 12 ? "AM" : "PM"}`;
      } else {
        document.getElementById("datepredict").innerText =
          "Date: Not enough data";
      }
      if (preds[1] != undefined) {
        document.getElementById("intensitypredict").innerText =
          `${cyc.label}: ~` + Math.round(preds[1] * 10) / 10;
      } else {
        document.getElementById("intensitypredict").innerText =
          `${cyc.label}: Not enough data`;
      }
    }
  } else {
...
function registerNewLog() {
  ...
  if (!isNaN(v)) {
    currentCycle.registerEvent(
      new Event(new Date(document.getElementById("time").value), v),
    );
    document.getElementById("invv").hidden = true;
    render();
    renderPopup(currentCycle);
...
function activateTrends() {
  ...
  for (let cycRes of analResults[currentCycle.group.id].trends[
    currentCycle.id
  ]) {
...
function closetrends() {
  ...
  document.getElementById("pottrends").innerText =
    `${currentCycle.events.length > 0 ? getNumberOfTrends(currentCycle) : "0"} potential trends found`;
  ...
  for (let key of order) {
    ...
    addStat(
      cycle.events.length > 0
        ? Math.round(
            (Analyze.extractIntensities(cycle.events).reduce((a, b) => a + b) /
              cycle.events.length) *
              10,
          ) / 10
        : "?",
...
window.addEventListener("load", () => {
  ...
  // in web version, check for if mobile.
  if (window.innerHeight == screen.height) {
    document.documentElement.style.setProperty("--pwa-top-offset", "30px");
    document.getElementById("privy").href = "https://trackyourcycles.com/privacy?inapp=true"
  }
});
...
container.addEventListener("dragend", (e) => {
  order = [...document.getElementById("cycles").children].map(
    (x) => x.dataset.id,
  );
  putToStorage();
});
...
function getDragAfterElement(container, x, y) {
  const draggableElements = [
    ...container.querySelectorAll(".cycle:not(.dragging)"),
  ];
  ...
  return draggableElements.reduce(
    (closest, child) => {
      ...
      if (
        ...
        Math.abs(offsetX) < closest.offsetX
      ) {
...
function toggleSettings() {
  ...
  // } catch (e) {
  //   document.getElementById("cachen").innerText = "mobile_build";
  // }
  ...
  for (let b of document.getElementsByTagName("button")) {
    if (b.id != "tut1") {
      ...
      b.addEventListener(
        ...
        () => {
          b.disabled = true;
          document.getElementById("tutText").innerText =
            "Tap on where it says New Cycle and give it a name. Maybe 'Coffee'?";
          document.getElementById("name").addEventListener(
            "blur",
            () => {
              document.getElementById("tutText").innerText =
                "Choose a group. We only look for relationships between cycles if they're in the same group.\n\nThen, tap on the underlined part of Average Intensity.";
              document.getElementById("avglabel").addEventListener(
                "focusin",
                () => {
                  document.getElementById("tutText").innerText =
                    "You can edit this to be whatever you want. Since this cycle is for coffee, maybe Quantity?";
                  document.getElementById("avglabel").addEventListener(
                    "blur",
                    () => {
                      document.getElementById("tutText").innerText =
                        "Great! Now tap Add Log to log your first cup of coffee.";
                      document.getElementById("tut2").disabled = false;
                      document.getElementById("tut2").addEventListener(
                        "click",
                        () => {
                          document.getElementById("tutText").innerText =
                            "For now, we'll just go with what's already there. Tap Save.";
                          document.getElementById("tut2").disabled = true;
                          document.getElementById("tut3").disabled = false;
                          document.getElementById("tut3").addEventListener(
                            "click",
                            () => {
                              document.getElementById("tutText").innerText =
                                "Great! That pink rectangle is a bar. When you add more logs, more bars will show up in the chart. You can tap the rectangle to delete a log. For now, tap below where it says X potential trends found.";
                              document.getElementById("pottrends").disabled =
                                false;
                              document
                                .getElementById("pottrends")
                                .addEventListener(
                                  "click",
                                  () => {
                                    document.getElementById(
                                      ...
                                      "Once the app finds some relationships, they'll show up here. The app will also make predictions below, where it says Prediction.\n\nThanks for taking part in this tutorial! Tap the Close button below Prediction to finish.";
                                    document.getElementById(
                                      "tutlast",
                                    ).disabled = false;
                                    document
                                      .getElementById("tutlast")
                                      .addEventListener(
                                        "click",
                                        () => {
